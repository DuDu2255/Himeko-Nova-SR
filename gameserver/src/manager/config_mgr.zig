...
pub const FreesrConfig = @import("../data/freesr_config.zig");
...
/// Guards `global_game_config_cache` between the KCP loop and the web thread,
/// which can replace the game config on an SRTools push.
pub var config_lock: std.Thread.Mutex = .{};
...
pub fn deinitGameGlobals() void {
    ...
    Lua.deinit(global_main_allocator);
}

pub fn UpdateGameConfig() !void {
    const fresh = try loadGameConfig(global_main_allocator);
    global_game_config_cache.game_config.deinit();
    global_game_config_cache.game_config = fresh;
}

/// Replaces the live game config, taking ownership of `next`.
pub fn ReplaceGameConfig(next: GameConfig.GameConfig) void {
    global_game_config_cache.game_config.deinit();
